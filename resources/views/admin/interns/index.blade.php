<x-dashboard-layout>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                                                ...
                                                <form action="{{ route('interns.destroy', $intern) }}" method="POST" class="inline">
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        data-delete-url="{{ route('interns.destroy', $intern) }}"
                                                        onclick="openDeleteModal('{{ $intern->id }}', '{{ $intern->user->name }}', this)">
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-900/75 hidden items-center justify-center z-50">
        <div class="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p class="text-gray-300 mb-6">Are you sure you want to delete the intern "<span id="internName" class="font-medium"></span>"? This action cannot be undone.</p>
    ...
    <script>
        function openDeleteModal(internId, internName, button) {
            const $modal = $('#deleteModal');
            const $deleteForm = $('#deleteForm');
            const $titleSpan = $('#internName');
               
            $deleteForm.attr('action', $(button).data('delete-url'));
            $titleSpan.text(internName);
